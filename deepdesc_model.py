...
class deepdescModel(BaseModel, torch.nn.Module):
    ...
    def forward(self, input_tensor=None):
        ...
        if input_tensor == None:
            ...
            self.deepfeats = self.netfeature_extractor(input_tensor)
...
def view_patches(files):
    plt.figure(figsize = (25,25))
    # gets the list of filenames for a clust
...
if __name__ == "__main__":
    ...
    fscmodel = deepdescModel(opt, mode="eval", num_classes=3)
    fscmodel.setup(opt)  # regular setup: load and print networks; create schedulers
    norm = nets.L2N()

    in_path = "/mnt/sda/plantation_cluster_test/256plantation_tiles"
    files = sorted(glob.glob(f"{in_path}/*.jp2"))
    subsample = random.sample(files, 10)
    subsample = ["/mnt/sda/plantation_cluster_test/256plantation_tiles/SSAr2_32628_GE01-QB02-WV02-WV03_PV_040_019_mosaic_11008_7936.jp2", "/mnt/sda/plantation_cluster_test/256plantation_tiles/SSAr2_32628_GE01-QB02-WV02-WV03_PV_040_019_mosaic_10752_7936.jp2",
                 "/mnt/sda/plantation_cluster_test/256plantation_tiles/SSAr2_32628_GE01-QB02-WV02-WV03_PV_040_019_mosaic_11008_7680.jp2", "/mnt/sda/plantation_cluster_test/256plantation_tiles/SSAr2_32628_GE01-QB02-WV02-WV03_PV_040_019_mosaic_11008_8192.jp2",
                 "/mnt/sda/plantation_cluster_test/256plantation_tiles/SSAr2_32628_GE01-QB02-WV02-WV03_PV_040_019_mosaic_11008_9216.jp2", "/mnt/sda/plantation_cluster_test/256plantation_tiles/SSAr2_32628_GE01-QB02-WV02-WV03_PV_040_019_mosaic_4608_10240.jp2"]
    
    view_patches(subsample)
    
    rasters_list = [np.array(rasterio.open(file).read(1),dtype=np.uint8) for file in subsample]
    rasters = np.swapaxes(np.array([np.dstack((img, img, img)) for img in rasters_list]), 3, 1)
        
    proto_patches = torch.from_numpy(rasters)
    #proto_patches = torch.randn(10, 3, 256, 256)
    #fscmodel.set_input({"input":proto_patches})
    #print(proto_patches.shape)
    proto_patches = proto_patches.float()
    o = fscmodel.forward(proto_patches)
    #print(o.shape)
    
    sim_proto = torch.matmul(o, o.t()).T
    print(sim_proto)    
    
    proto_desc = norm(torch.mean(o, dim=0)[None, ])
    print(proto_desc.shape)
    target_patches = torch.randn(64, 3, 256, 256)
    #fscmodel.set_input({"input":target_patches})
    target_descs = fscmodel.forward(target_patches)
    print(target_descs.shape)
